...
| 孫尚香 | 梟姬 | 失去裝備（被拆 / 被順 / **主動換裝蓋掉舊裝備** / 被反饋取走 / 被麒麟弓棄馬）自動摸 2；per-card，一次失去 N 張摸 2N |
...
- 梟姬已覆蓋：被過河拆橋拆裝備、被順手牽羊偷裝備、主動換裝（武器 / 防具 / ±1 馬）、
  被反饋取走裝備（取手牌不算）、被麒麟弓棄馬（只有一匹直接移除 / 兩匹由攻擊方選一匹）。
  麒麟弓的摸牌事件排在**傷害事件之前**（官方順序是先失去裝備再造成傷害）。
  尚未覆蓋（follow-up，逐條列出取代原本籠統備註）：
  借刀殺人奪武器、貫石斧代價棄裝備、制衡棄裝備、主公殺忠臣棄光裝備。
